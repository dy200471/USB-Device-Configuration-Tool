...
static void put_le(std::vector<uint8_t>& b, size_t pos, uint64_t v, int bytes) {
...
std::vector<uint8_t> build_frame(uint8_t command,
                                 ...
                                 uint8_t version) {
    std::vector<uint8_t> buf(CONFIG_SIZE, 0);
    buf[0] = version;
    buf[1] = command;
    size_t pos = 2;
    for (const auto& f : fields) {
        switch (f.type) {
            case FType::U8:
            case FType::I8:
                put_le(buf, pos, static_cast<uint64_t>(f.value) & 0xFF, 1);
                pos += 1;
                break;
            case FType::U16:
                put_le(buf, pos, static_cast<uint64_t>(f.value) & 0xFFFF, 2);
                pos += 2;
                break;
            case FType::U32:
            case FType::I32:
                put_le(buf, pos, static_cast<uint64_t>(static_cast<uint32_t>(f.value)), 4);
                pos += 4;
                break;
        }
    }
    uint32_t crc = crc32(buf.data(), CONFIG_SIZE - 4);
    put_le(buf, CONFIG_SIZE - 4, crc, 4);
    return buf;
}

bool check_crc(const uint8_t* data, int size) {
    uint32_t stored = static_cast<uint32_t>(data[size - 4]) |
                      (static_cast<uint32_t>(data[size - 3]) << 8) |
                      (static_cast<uint32_t>(data[size - 2]) << 16) |
                      (static_cast<uint32_t>(data[size - 1]) << 24);
    return stored == crc32(data, size - 4);
}
